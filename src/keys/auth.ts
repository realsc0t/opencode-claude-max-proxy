...
function defaultAdminPath(): string {
  return resolve(homedir(), ".meridian", "admin.json")
}

let config: AdminConfig | null = null
let filePath = env("ADMIN_FILE") ?? defaultAdminPath()
...
/** Reset admin config — for testing only. */
export function resetAdminConfig(): void {
  config = null
  filePath = env("ADMIN_FILE") ?? defaultAdminPath()
}
